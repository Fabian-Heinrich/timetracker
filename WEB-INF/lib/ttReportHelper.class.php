...
class ttReportHelper {
  ...
  static function getItems($options) {
    ...
    if ($options['show_billable'])
      array_push($fields, 'l.billable as billable');
    // Prepare sql query part for left joins.
    $left_joins = null;
    if ($options['show_client'] || $options['show_client_number'] || $grouping_by_client)
    ...
    if ($options['show_cost'] && $user->isPluginEnabled('ex')) { // if ex(penses) plugin is enabled
      ...
      if ($options['show_client'] || $options['show_client_number'] || 'client' == $grouping_by_client)
  ...
  static function getReportOptions($bean) {
	...
	$options['show_billable'] = $bean->getAttribute('chbillable');	
	$options['show_client_number'] = $bean->getAttribute('chclientnumber');
